...
function PublicTransportchange(PublicTransport, PersonType,vehicleType) {
    ...
    if (PublicTransport * 1 == 0) {
        ...
        getDropDownDataWithSelectedValue(VehicleType.attr('id'), 'Select Type', '/EMN/GetVehicleTypes?TypeVal=' + PersonType, vehicleType);
    } else {
        $('.forYes').show();
        $('.forNo').hide();
        $("#VehicleType").val('');
...
async function GetTourCategory(PublicTransport) {
    (async function () {
        const r1 = await getMultiselectData('TourCategory', '/EMN/GetTourCategories?PTval=' + PublicTransport);
    })();
...
async function getInitialData() {
    ...
    $.ajax({
        url: '/EMN/GetTraveelingDetailsReverseData',
        method: 'GET',
        dataType: 'json',
        success: function (data) {
            $(data).each(function (index, item) {
