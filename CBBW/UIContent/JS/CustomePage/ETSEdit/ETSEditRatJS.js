    ...
    $.ajax({
        ...
        success: function (data) {
            $(data).each(function (index, item) {
                ...
                $('#AppStat2').val(item.IsApprovedDisplay);
                $('#AppDateTime2').val(item.AppDateTimeDisplay);
                $('#AppReason2').val(item.NotAppReason);
...
$(document).ready(function () {
    ...
    $('#btnBack').click(function () {
        ...
        var backurl = "/Security/ETSEdit/RatificationIndex";
        if (backbtnactive == 1) {
            Swal.fire({
                title: 'Confirmation',
