...
namespace CBBW.DAL.DataSync
{
    public partial class CommonTable
    {
        ...
        public CommonTable(List<UserRole> customoptions)
        {
            try
            {
                UDTable = new DataTable();
                UDTable.Columns.Add("sRoleID", typeof(string));
                UDTable.Columns.Add("sRoleName", typeof(string));
                UDTable.Columns.Add("iLocationTypeCode", typeof(int));
                ...
                if (customoptions != null && customoptions.Count > 0)
                {
                    foreach (UserRole obj in customoptions)
                    {
                        DataRow dr = UDTable.NewRow();
                        dr["sRoleID"] = obj.RoleID;
                        dr["sRoleName"] = obj.RoleName;
                        dr["iLocationTypeCode"] = obj.LocationTypeCode;
                        dr["iCentreCode"] = obj.LocationCode;
